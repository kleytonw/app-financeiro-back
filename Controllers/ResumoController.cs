...
namespace ERP_API.Controllers
{
    ...
    public class ResumoController(Context context) : ControllerBase
    {
        [HttpGet]
        [Route("grafico-pizza")]
        public IActionResult GraficoPizza(int mes, int ano)
        {
            var result = context.Transacao
                .Include(x => x.Dependente)
                .Where(x => x.Situacao == "Ativo" && x.DataCompra.Month == mes && x.DataCompra.Year == ano)
                .AsNoTracking()
                .GroupBy(x => x.Dependente.Nome)
                .Select(g => new
                {
                    NomeDependente = g.Key,
                    TotalGasto = g.Sum(x => x.Valor)
                })
                .OrderByDescending(x => x.TotalGasto)
                .ToList();

            var totalMes = result.Sum(x => x.TotalGasto);

            return Ok(new { totalMes, dependentes = result });
        }
        ...
        [HttpGet]
        [Route("listar-por-cartao")]
        public IActionResult ListarPorCartao(int idCartao)
        {
            var result = context.Transacao.Include(x => x.Categoria)
                .Include(x => x.Cartao)
                .Include(x => x.Dependente)
                .Where(x => x.Situacao == "Ativo" && x.IdCartao == idCartao)
                .AsNoTracking()
                .Select(m => new
                {
                    m.IdTransacao,
        ...
        [HttpGet]
        [Route("listar-por-dependente")]
        public IActionResult ListarPorDependente(int idDependente)
        {
            var result = context.Transacao.Include(x => x.Categoria)
                .Include(x => x.Cartao)
                .Include(x => x.Dependente)
                .Where(x => x.Situacao == "Ativo" && x.IdDependente == idDependente)
                .AsNoTracking()
                .Select(m => new
                {
                    m.IdTransacao,
        ...
        public IActionResult ListarPorCategoria(int idCategoria)
        {
            var result = context.Transacao.Include(x => x.Categoria)
                .Include(x => x.Cartao)
                .Include(x => x.Dependente)
                .Where(x => x.Situacao == "Ativo" && x.IdCategoria == idCategoria)
                .AsNoTracking()
                .Select(m => new
                {
                    m.IdTransacao,
        ...
        public IActionResult Salvar([FromBody] TransacaoRequest model)
        {
            ...
            else if (model.NumeroParcelas > 0)
            {
                transacao = null;
